    ...
    const loadInbox = async () => {
      ...
      try {
        const response = await requestMemberApi(
          `/messages/inbox?user_id=${encodeURIComponent(userId)}`,
          {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
          setError(response?.data?.message || "Unable to load inbox messages.");
          setRows([]);
          return;
        }
          ...
          <div className="flex-1 min-w-0 flex flex-col overflow-x-hidden">
            ...
            {selectedMessage && (
              <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4">
                <p className="text-sm"><span className="font-semibold">From:</span> {selectedMessage.from || "-"}</p>
