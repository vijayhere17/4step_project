...
const GroupBuiltupBonus = () => {
  ...
  useEffect(() => {
    ...
    const fetchRows = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/bonuses/group-builtup${query}`, {
          method: "GET",
          headers: {
        ...
        if (!response.ok) {
          throw new Error(data?.message || "Unable to fetch group builtup bonus data");
        }
        ...
        if (isMounted) {
          setError(fetchError.message || "Unable to fetch group builtup bonus data");
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
  ...
  const walletText = (status) => {
    return String(status || "").toLowerCase() === "paid" ? "Credited" : "On Hold";
  };
                ...
                <tbody>
                  {!isLoading && rows.length === 0 && (
                    <tr className="border-b">
                      <td className="p-4" colSpan={10}>No Group Builtup bonus data found</td>
                    </tr>
                  )}
